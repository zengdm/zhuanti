define(function(require, exports, module) {
    ...
    var UPLOADCARPROVE = form.domainUrl+'/passport/ark/uploadCarProve';
    ...
    var buy = {
                                ...
                                xhr.open("POST", url[1], true); //post方式，url为服务器请求地址，true 该参数规定请求是否异步处理。
                                  xhr.onreadystatechange = function() {
                                    if (xhr.readyState === 4) {
                                        ...
                                        if(types == 'left'){
                                            obj.id_card_url = result.img_url;
                                        }else{
                                            obj.car_invoice_url = result.img_url;
                                        }
        ...
        uploadSubmit:function(){
            ...
            $(obj.uploadSubmit).on('click',function(){
                 ...
                 if(!obj.car_invoice_url){
                    tip.info(obj.invoice);
                    return false
                 }
                    var opt = {
                        uid: obj.uid,
                        id_card_url: obj.id_card_url,
                        car_invoice_url: obj.car_invoice_url,
                        joinid:obj.joinid       
                    }
                    console.log(opt);
                   $.ajax({
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.code == 0) {
                            console.log(res);
                            that.verifyTimer();
                            window.location.href = "//zhuanti.diandong.com/zt/2018/07/MG/mobile/buy-upload.html?uid="+res.data.uid+'&joinid='+res.data.joinid;
                            that.initSubmit();
                        } else {
                            tip.info(res.message);
                        }
