 ...
 * @date 2015/09/29
 * @param init(parent), 初始化页面非弹出层试驾，parent 最外层jq对象
 * @param show(brand<品牌id>, series<车系id>, model<车型id>, activityId<活动id>), 预约试驾弹出层
 */
define(function (require, exports, module) {
	...
	drive = {
		...
		setWrapperLocation: function(){
			...
			$(selectors.drivebox, wrapper).css({
				top: top
			});
		},
		// 设置经销商
		...
		submitInfo: function(parent){
			...
			if (date.getTime() < cDate && $(s.shuttle, parent)[0].checked) {
				tip.error('【' + month + '月' + day + '日】之后可为您提供免费专车服务');
				return;
			}
