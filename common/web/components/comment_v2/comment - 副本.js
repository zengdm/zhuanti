	...
	var article = {
		...
		bindEvent: function() {
			...
			$('.comment-holder').on('click', '.comment-like-btn', function() {
				...
				if ($(that).find('i').hasClass('comment-praise')) {
				...
				$.ajax({
					...
					success: function(res) {
						console.log(res)
						if (res.code == 0) {
							$(that).find('i').addClass('comment-praise');
							$(that).find('span').addClass('comment-praise');
							var praiseNum = parseInt($(that).find('span').html());
							$(that).find('span').html(praiseNum + 1)
